...
export function SimProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Modo screenshot: ?seed=alerta fuerza el estado de alerta.
    const seed = new URLSearchParams(window.location.search).get("seed");
    if (seed === "alerta") {
      setState({
        ...initialState,
        demoMode: "alerta",
        events: initialState.events.map((e) =>
          e.moment === "desayuno" ? { ...e, state: "omitido", confirmedAt: undefined } : e,
        ),
      });
      setHydrated(true);
      return;
